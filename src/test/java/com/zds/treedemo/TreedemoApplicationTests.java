...
//		orgRecursion(childList,tagCategoryDOS,68);
    ...
    public void testTree() throws Exception {
        // 获取所有体系的的标签分类
        List<TagCategoryDO> all = tagCategoryMapper.selectAll();
        // 传入你想递归查询的标签分类id
        Integer id = 68;
        // 某一个标签分类的子分类
        ArrayList<TagCategoryDO> childList1 = new ArrayList<>();

        for (TagCategoryDO tagCategoryDO : all) {
            // 遍历所有的分类 找到以这个id 为parent_id的子分类 加入到这个子分类
            if (id.equals(tagCategoryDO.getParentId())) {
                childList1.add(tagCategoryDO);
            }
        }

//        ArrayList<TagCategoryDO> childList2 = new ArrayList<>();
//        childList1.forEach(item -> {
//            for (TagCategoryDO tagCategoryDO : all) {
//                // 遍历所有的分类 找到以这个id 为parent_id的子分类 加入到这个子分类
//                if (item.getId().equals(tagCategoryDO.getParentId())) {
//                    childList2.add(tagCategoryDO);
//                }
//            }
//        });


//		childList2.forEach(item->{
//			System.out.println("---->>>>"+item);
//		});
...
//            node.setChildren(list);
    ...
    // 获取某一个节点的相邻子节点 但是没法往下递归
    public static List<TagCategoryDO> getAllNewsClass1(List<TagCategoryDO> allList, int pid, List<TagCategoryDO> childList) {
        for (TagCategoryDO tagCategoryDO : allList) {
            // 递归的终止条件  所有的标签分类 没有一个是以传入的id 作为parentid 的
            ...
            if (Integer.valueOf(tagCategoryDO.getParentId()) == pid) {
                ...
                getAllNewsClass1(allList, tagCategoryDO.getId(), childList);
            }
        }
        return childList;
    }
